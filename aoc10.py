...
# I start at S
def go(world, pos, dn):
    # dict that maps the positions in the loop to the direction they arrived to the position
    loop = {}
    c = 0
    nextpos = pos
    d = dn
...
#        cpos = nextpos
        ...
        if d in 'ud':
            ...
            updown = part_types[world[nextpos[0]][nextpos[1]]][d]
        loop[nextpos] = updown
...
def count_enclosed(world, loop):
    ...
    for i in range(len(world)):
        ...
        for j in range(len(world[i])):
            if (i,j) in loop:
                d = loop[(i,j)]
                if '|' == world[i][j] or 'S' == world[i][j]:
                    c += 1
                elif '-' == world[i][j]:
                    pass
                else:
#                    dn = opp_dir[part_types[world[i][j]][d]]
                    if None == prev:
                        prev = d
                        c += 1
                    elif prev == d:
